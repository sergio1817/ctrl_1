...
Sliding::Sliding(const LayoutPosition *position, string name): ControlLaw(position->getLayout(), name, 4){
    ...
    GroupBox *ac11 = new GroupBox(reglages_groupbox->NewRow(), "AC_ori1");
...
void Sliding::UseDefaultPlot5(const LayoutPosition *position) {    
    DataPlot1D *Sq = new DataPlot1D(position, "S_qa", -3, 3);
...
void Sliding::UseDefaultPlot6(const LayoutPosition *position) {    
    ac1->UseDefaultPlot(position);
}

void Sliding::UseDefaultPlot7(const LayoutPosition *position) {    
